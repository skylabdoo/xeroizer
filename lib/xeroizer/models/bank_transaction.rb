...
module Xeroizer
  module Record
    ...
    class BankTransaction < Base
      ...
      alias reconciled? is_reconciled
      ...
      def sub_total=(_value)
        raise SettingTotalDirectlyNotSupported.new(:sub_total)
      end

      def total_tax=(_value)
        raise SettingTotalDirectlyNotSupported.new(:total_tax)
      end

      def total=(_value)
        raise SettingTotalDirectlyNotSupported.new(:total)
      end

      def total = sub_total + total_tax

      def sub_total
        if ought_to_recalculate_totals?
          result = LineItemSum.sub_total(line_items)
      ...
      def total_tax
        ought_to_recalculate_totals? ? LineItemSum.total_tax(line_items) : attributes[:total_tax]
      end
