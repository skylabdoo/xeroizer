      ...
      module InstanceMethods
        ...
        def parse_where_hash(where)
          ...
          where.each do |key, value|
            ...
            (_, field) = model_class.fields.find { |_k, v| v[:internal_name] == attribute_name }
            raise InvalidAttributeInWhere.new(model_name, attribute_name) unless field
        ...
        def where_condition_part(field, expression, value)
          ...
          when :datetime    then [field[:api_name], expression,
                                  "DateTime.Parse(\"#{value.utc.strftime('%Y-%m-%dT%H:%M:%S')}\")"]
          when :datetime_utc then [field[:api_name], expression,
                                   "DateTime.Parse(\"#{value.utc.strftime('%Y-%m-%dT%H:%M:%S')}\")"]
          when :belongs_to
          when :has_many
          when :has_one
