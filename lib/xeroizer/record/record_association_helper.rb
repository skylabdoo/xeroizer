    ...
    module RecordAssociationHelper
      ...
      module ClassMethods
          ...
          define_method "add_#{internal_singular_field_name}" do |*args|
            ...
            records.each do |record|
              ...
              unless record.is_a?(record_class)
                raise XeroizerError.new("Record #{record.class.name} is not a #{record_class.name}.")
              end

              attributes[field_name] ||= []
              attributes[field_name] << record
              last_record = record
            end
        ...
        def define_association_attribute(field_name, internal_field_name, association_type, options)
          define_simple_attribute(field_name, association_type, options.merge!(skip_writer: true),
                                  (association_type == :has_many ? [] : nil))

          internal_field_name = options[:internal_name] || field_name
          internal_singular_field_name = options[:internal_name_singular] || internal_field_name.to_s.singularize
          model_name = options[:model_name] ? options[:model_name].to_sym : field_name.to_s.singularize.camelize.to_sym
          define_method :"#{internal_field_name}=" do |value|
            record_class = (options[:base_module] || Xeroizer::Record).const_get(model_name)
            case value
            when Hash
              attributes[field_name] = (association_type == :has_many ? [] : nil)
              case association_type
              when :has_many
                attributes[field_name] = []
                send(:"add_#{internal_singular_field_name}", value)

              when :belongs_to
                attributes[field_name] =
                  (options[:base_module] || Xeroizer::Record).const_get(model_name).build(value,
                                                                                          new_model_class(model_name))
            ...
            when Array
              ...
              value.each do |single_value|
                case single_value
                when Hash         then send(:"add_#{internal_singular_field_name}", single_value)
                when record_class then attributes[field_name] << single_value
                else                   raise AssociationTypeMismatch.new(record_class, single_value.class)
                end
              end

            when record_class
              attributes[field_name] = (association_type == :has_many ? [value] : value)
          ...
          return unless list_contains_summary_only?

          define_method internal_field_name do
            unless new_record? || options[:list_complete] || (options[:complete_on_page] && paged_record_downloaded?) || complete_record_downloaded?
              download_complete_record!
            end
            attributes[field_name] || (association_type == :has_many ? [] : nil)
          end
        end
