      ...
      module ClassMethods
        ...
        def build_from_node(node, parent, base_module)
          ...
          node.elements.each do |element|
            field = fields[element.name.to_s.underscore.to_sym]
            next unless field
                    ...
                    when :has_many
                      if element.element_children.size > 0
                        ...
                        element.children.inject([]) do |list, inner_element|
                          list << base_module.const_get(sub_field_name).build_from_node(inner_element, sub_parent,
                                                                                        base_module)
                        end
                      end
      ...
      module InstanceMethods
        ...
        def to_xml(b = Builder::XmlMarkup.new(indent: 2))
          optional_root_tag(parent.class.optional_xml_root_name, b) do |c|
            c.tag!(model.class.xml_node_name || model.model_name) do
              attributes.each do |key, value|
                field = self.class.fields[key]
                value = send(key) if field[:calculated]
        ...
        #     </Payment>
        #   </Payments>
        def optional_root_tag(root_name, b, &)
          if root_name
            b.tag!(root_name, &)
          else
            yield(b)
        ...
        def xml_value_from_field(b, field, value)
          ...
          when :datetime
            real_value = case value
                         when Time, DateTime then value.utc.strftime('%Y-%m-%dT%H:%M:%S')
                         when NilClass       then nil
                         else raise ArgumentError.new("Expected Time or DateTime object for the #{field[:api_name]} field")
                         end
            b.tag!(field[:api_name], real_value)
          when :belongs_to
            value.to_xml(b)
