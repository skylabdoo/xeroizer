...
class TaxRateTest < Minitest::Test
  ...
  should 'build and save a tax rate with components via PUT' do
    @client.expects(:http_post).with do |_client, url, body, _extra_params|
      url == 'https://api.xero.com/api.xro/2.0/TaxRates' &&
        body == expected_tax_rate_create_body
    end.returns(tax_rate_create_successful_response)
  ...
  should 'allow reading of the ReportTaxType' do
    @client.expects(:http_get).with do |_client, url, _extra_params|
      url == 'https://api.xero.com/api.xro/2.0/TaxRates'
    end.returns(tax_rate_get_response)
  ...
  should 'allow GET with a filter on ReportTaxType' do
    ...
    @client.expects(:http_get).with do |_client, url, extra_params|
