...
VCR_RECORD_MODE = case ENV.fetch('VCR_MODE', nil)
                  when 'all'           then :all
...
if VCR_RECORD_MODE != :none
  ...
  abort "VCR_MODE=#{ENV.fetch('VCR_MODE', nil)} records live; set #{missing.join(', ')} first." unless missing.empty?
end
...
VCR.configure do |c|
  ...
  # are harmless.
  c.filter_sensitive_data('<XERO_CLIENT_ID>')     { ENV.fetch('XERO_CLIENT_ID', nil) }
  c.filter_sensitive_data('<XERO_CLIENT_SECRET>') { ENV.fetch('XERO_CLIENT_SECRET', nil) }
  c.filter_sensitive_data('<XERO_TENANT_ID>')     { ENV.fetch('XERO_TENANT_ID', nil) }
  c.filter_sensitive_data('<XERO_ACCESS_TOKEN>')  { ENV.fetch('XERO_ACCESS_TOKEN', nil) }
